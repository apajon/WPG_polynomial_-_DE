...
%     addpath ./FEM
    ...
    time=(0:sum(discretization))/frequency;
...
%     for j = 1
        ...
        foot_step_coor=pstep(foot_step_wanted,:);

        %% %compute zmp under one foot
        [time_ pzmp pcom fzmp]=zmp_under_foot(foot_step_wanted,nbpankle,time,xpzmp,ypzmp,xpzmp1,ypzmp1,xpzmp2,ypzmp2,xpcom,ypcom,z*ones(size(xpcom)),zfzmp1,xAfcom,xBfcom,yAfcom,yBfcom,psa_abcd,discretization,discretization_,mg);
        %%
        theta_=psi(1:end-1);
...
%         [Zdes,Fdes] = changeRefxxx(ZMPx, ZMPy, Fx, Fy, Fz);
        ...
        rightorleft=(-1)^(foot_step_wanted+1+firstSS);%+1 for right foot and -1 for left foot

        [Zdes,Fdes] = changeRef(ZMPx, ZMPy, Fx, Fy, Fz,backtoankle,fronttoankle,exttoankle,inttoankle,xpankle,ypankle,rightorleft);
...
%             fzmp_r=[fzmp_r;[Fdes(1:2,:)'*inv(mrot) Fdes(3,:)'];zeros(discretization(2),3)];
            fet1_r=[fet1_r;ones(length(Fdes(3,:)'),3);zeros(discretization(2),3)];
            fet2_r=[fet2_r;ones(length(Fdes(3,:)'),3);zeros(discretization(2),3)];
            fet3_r=[fet3_r;ones(length(Fdes(3,:)'),3);zeros(discretization(2),3)];
            fet4_r=[fet4_r;ones(length(Fdes(3,:)'),3);zeros(discretization(2),3)];
            tpp_r=[tpp_r;ones(length(Fdes(3,:)'),3);zeros(discretization(2),3)];
            fzmp_r=[fzmp_r;[Fdes(1:2,:)'*inv(mrot) Fdes(3,:)'];zeros(discretization(2),3)];
        else
%             orig_foot=[-backtoankle +(Lfooty/2-inttoankle)]*inv(mrot);
%             orig_foot=repmat(orig_foot,size(cost.fet1(:,1),1),1);
...
%             fzmp_l=[fzmp_l;[Fdes(1:2,:)'*inv(mrot) Fdes(3,:)'];zeros(discretization(2),3)];
            fet1_l=[fet1_l;ones(length(Fdes(3,:)'),3);zeros(discretization(2),3)];
            fet2_l=[fet2_l;ones(length(Fdes(3,:)'),3);zeros(discretization(2),3)];
            fet3_l=[fet3_l;ones(length(Fdes(3,:)'),3);zeros(discretization(2),3)];
            fet4_l=[fet4_l;ones(length(Fdes(3,:)'),3);zeros(discretization(2),3)];
            tpp_l=[tpp_l;ones(length(Fdes(3,:)'),3);zeros(discretization(2),3)];
            fzmp_l=[fzmp_l;[Fdes(1:2,:)'*inv(mrot) Fdes(3,:)'];zeros(discretization(2),3)];
...
%%
% 
% save('result_FEM_06_15_oscillation_tss800_tds400_320000','fet1_r_','fet2_r_','fet3_r_','fet4_r_','tpp_r_','fet1_l_','fet2_l_','fet3_l_','fet4_l_','tpp_l_')
