function [mdt] = compute_coeff_dt_angle_matrix(discretization,frequency,nbphases,nbpointdiscret,type_phase)

% cutting=0.5;

mdt=zeros(nbpointdiscret,nbphases*6+3+3);

%%%zmp1 init cut in two%%%
nb=0;
% rowinitphase=1+discretization(1);
% for i=1:ceil(nb*cutting)
%     dt=i/frequency;%dt is delta(tj)=t-Tj
%     Dt=[dt^0 dt^1 dt^2 dt^3];
%     mdt(rowinitphase+i,(1-1)*4+1:(1-1)*4+4)=Dt;
% end
% rowinitphase=rowinitphase+ceil(nb*cutting);
% for i=1:nb-ceil(nb*cutting)
%     dt=i/frequency;%dt is delta(tj)=t-Tj
%     Dt=[dt^0 dt^1 dt^2 dt^3];
%     mdt(rowinitphase+i,(2-1)*4+1:(2-1)*4+4)=Dt;
% end

rowinitphase=1+discretization(1);
%%%generation of dt%%%
for j=2:nbphases-1
    rowinitphase=rowinitphase+nb;
    nb=discretization(j);
    if(type_phase(j)~=0)
        for i=1:nb
            dt=i/frequency;%dt is delta(tj)=t-Tj
            Dt=[dt^0 dt^1 dt^2 dt^3 dt^4 dt^5];
            mdt(rowinitphase+i,(j+1-1)*6+1:(j+1-1)*6+6)=Dt;
        end
    end
end

%%%zmp1 fin cut in two%%%
% rowinitphase=rowinitphase+nb;
% nb=discretization(end-1);
% for i=1:ceil(nb*cutting)
%     dt=i/frequency;%dt is delta(tj)=t-Tj
%     Dt=[dt^0 dt^1 dt^2 dt^3];
%     mdt(rowinitphase+i,end-7:end-4)=Dt;
% end
% rowinitphase=rowinitphase+ceil(nb*cutting);
% for i=1:nb-ceil(nb*cutting)
%     dt=i/frequency;%dt is delta(tj)=t-Tj
%     Dt=[dt^0 dt^1 dt^2 dt^3];
%     mdt(rowinitphase+i,end-3:end)=Dt;
% end

end